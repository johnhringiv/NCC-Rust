...
fn get_sandler_cases() -> Vec<TestCase>{
    ...
    let expected_results = load_expected("../writing-a-c-compiler-tests/expected_results.json").unwrap_or_else(|_| HashMap::new());
    let mut cases = vec![];
    
    for entry in glob("../writing-a-c-compiler-tests/tests/**/*.c").expect("glob err").flatten() {
        let chapter = entry.to_str().and_then(|s| s.split("chapter_").nth(1)).and_then(|s| s.split('/').next()).and_then(|s| s.parse::<i32>().ok()).unwrap_or(0);
...
fn run_test(case : &TestCase, total_passed: &mut usize, total_failed: &mut usize, failed_tests: &mut Vec<String>, iced: bool) {
    ...
    let actual = if compile_output.status.success() { // compilation succeed
        let run_output = std::process::Command::new(binary_path_str)
            .output().unwrap();
